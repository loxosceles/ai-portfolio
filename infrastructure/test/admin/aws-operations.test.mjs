...
describe('AWSOperations', () => {
  ...
  beforeEach(() => {
    ...
    // DEBUG: Reset the mock to catch-all first
    ssmMock.onAnyCommand().rejects(new Error('UNMOCKED SSM CALL in test'));

    // Then add specific mocks
    ssmMock
      .on(GetParameterCommand, { Name: '/portfolio/dev/DEVELOPER_TABLE_NAME' })
      .resolves({ Parameter: { Value: 'dev-developers-table' } });
    ssmMock
      .on(GetParameterCommand, { Name: '/portfolio/dev/PROJECTS_TABLE_NAME' })
      .resolves({ Parameter: { Value: 'dev-projects-table' } });
    ssmMock
      .on(GetParameterCommand, { Name: '/portfolio/dev/RECRUITER_PROFILES_TABLE_NAME' })
      .resolves({ Parameter: { Value: 'dev-recruiters-table' } });

    awsOps = new AWSOperations(ADMIN_CONFIG);
  });

  test('DEBUG: simple mock test', async () => {
    // Reset and setup simple mock - use .rejects() not .reject()
    ssmMock.reset();
    ssmMock.onAnyCommand().rejects(new Error('UNMOCKED SSM CALL in test'));

    const debugAwsOps = new AWSOperations(ADMIN_CONFIG);

    console.log('=== SIMPLE DEBUG TEST ===');

    // Test direct call
    try {
      const result = await debugAwsOps.ssmClient.send(
        new GetParameterCommand({ Name: '/test/path' })
      );
      console.log('Direct call succeeded:', result.Parameter.Value);
    } catch (error) {
    ...
    console.log('Mock calls recorded:', calls.length);
  });
  ...
  test('should get all items from DynamoDB table', async () => {
    ...
    ddbMock.on(ScanCommand, { TableName: 'dev-developers-table' }).resolves({
      Items: [
  ...
  describe('CRUD Operations', () => {
    ...
    describe('Business Rules', () => {
      test('should prevent creating items in developer table', async () => {
        await expect(awsOps.createItem('dev', 'developer', { id: 'dev1' })).rejects.toThrow(
          'Cannot create new items in developer table. Use updateItem instead.'
        );
      });

      test('should prevent deleting from developer table', async () => {
        await expect(awsOps.deleteItem('dev', 'developer', {})).rejects.toThrow(
          'Cannot delete from developer table.'
        );
      });
      ...
      test('should require existing developer profile for updates', async () => {
        ...
        await expect(awsOps.updateItem('dev', 'developer', { id: 'dev1' })).rejects.toThrow(
          'No developer profile exists. Create one through data import first.'
        );
      });
      ...
      test('should allow developer update when profile exists', async () => {
        ...
        await expect(awsOps.updateItem('dev', 'developer', { id: 'dev1' })).resolves.not.toThrow();
      });
    });
    ...
    describe('Helper Functions', () => {
      test('should generate correct keys for different table types', () => {
        expect(awsOps.getItemKey('recruiters', { linkId: 'rec1' })).toEqual({ linkId: 'rec1' });

        expect(awsOps.getItemKey('projects', { id: 'proj1' })).toEqual({ id: 'proj1' });

        expect(() => awsOps.getItemKey('unknown', {})).toThrow(
          'Cannot generate key for table: unknown'
        );
      });
    });
  });
  ...
  test('should get individual SSM parameter', async () => {
    ...
    ssmMock
      .on(GetParameterCommand, { Name: '/portfolio/prod/DATA_BUCKET_NAME' })
    ...
    const calls = ssmMock.commandCalls(GetParameterCommand);
    const paramCall = calls.find(
      (call) => call.args[0].input.Name === '/portfolio/prod/DATA_BUCKET_NAME'
    );
